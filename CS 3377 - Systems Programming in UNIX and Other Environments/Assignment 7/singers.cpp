...
void GetLyricLinesFromFileToVector(string          lyricsFilenameStr,
    ...
    ifstream file(lyricsFilenameStr);
...
void* SingLinesThread(void* singLinesThreadInfoStructPtr) {
    ...
    cout << "Singer " << *singersNoIdPtr << " started singing!" << endl;
    ...
    cout << "Singer " << *singersNoIdPtr << " finished singing!" << endl;
...
int main(int argc, char* argv[]) {
  ...
  try{
    ...
    if (argc !=3) {
        string errorStr = "Usage : " + string(argv[0]) + " <lyrics file> <number of singers>";
            throw domain_error(LineInfo("open FAILURE", __FILE__, __LINE__));
    }
    ...
    cout << endl << endl;
    //$$
    cout << "Lyrics file " << lyricsFilenameStr << " has " << noOfLyricLines << " lines." << endl;
    cout << endl << endl;
    ...
    for (unsigned singerNo = 0; singerNo < noOfSingers; ++singerNo) {
        ...
        *singerNoIdPtr = singerNo+1;
        ...
        // display create thread with *singerNoIdPtr
         cout << "Create thread with singer " << *singerNoIdPtr << endl;

        // $$ unlock the mutex
         pthread_mutex_unlock(&lock);
        
        //$$ create the thread using function SingLinesThread
        if (pthread_create(&singersThreadIdPtr[singerNo], NULL, &SingLinesThread, &singLinesThreadInfoStructPtr[singerNo]) != 0) {
    ...
    for (unsigned singerNo = 0; singerNo < noOfSingers; ++singerNo) {
        if (pthread_join(singersThreadIdPtr[singerNo], NULL) != 0) {
